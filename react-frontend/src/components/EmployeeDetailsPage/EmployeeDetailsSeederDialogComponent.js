...
import seederData from "./employeeDetails.seeder.json";
...
const EmployeeDetailsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("employeeDetails").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(EmployeeDetailsSeederDialogComponent);
