...
suitableJobs: _entity?.suitableJobs,
        ...
        await client.service("jobClassified").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info jobClassified updated successfully" });
...
const employeeInterestOptions = employeeInterest.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit JobClassified" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="jobClassified-edit-dialog-component">
