...
import seederData from "./jobClassified.seeder.json";
...
const JobClassifiedSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("jobClassified").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(JobClassifiedSeederDialogComponent);
